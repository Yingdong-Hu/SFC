...
def load_image(img_path):
    ...
    img = img / 255.0
    img = img[:, :, ::-1]  # convert from BGR mode to RGB mode
    img = img.copy()
...
class VOSDataset(torch.utils.data.Dataset):
    def __init__(self, args):
        ...
        self.imgSize = args.imgSize  # -1
        self.videoLen = args.videoLen  # 20
        self.mapScale = args.mapScale  # [8, 8] (downscale)

        self.texture = args.texture  # False
        self.round = args.round  # False
        self.use_lab = getattr(args, 'use_lab', False)  # False

        self.filelist = self.filelist.replace('~', home)
        ...
        for line in f:
            rows = line.split()
            jpgfile = rows[0].replace('~', home)
            lblfile = rows[1].replace('~', home)
    ...
    def __getitem__(self, index):
        ...
        for i, p in enumerate(lbl_paths):
            ...
            if onehot is None:
                ...
                tmp = [np.all(lbls[i] == ll, axis=-1) for ll in lblset]
